/* Cart page logic – uses localStorage plato_cart from main.js */
const CATALOG = {
...
function getCartRaw() {
  try { return JSON.parse(localStorage.getItem('plato_cart') || '[]'); }
  catch { return []; }
}
...
function saveCartRaw(items) {
  ...
  if (typeof updateBadges === 'function') {
    // sync with main.js global if available
    try {
      cart.length = 0;
...
/** Aggregate cart: { id, qty }[] */
function getAggregated() {
  const raw = getCartRaw();
  const map = {};
  raw.forEach(item => {
    const id = String(item.id);
    if (!map[id]) map[id] = { id, qty: 0 };
    map[id].qty += 1;
  });
...
function setQty(id, qty) {
  ...
  for (let i = 0; i < qty; i++) {
    raw.push({ id: String(id), name: info.name, time: Date.now() });
  }
  saveCartRaw(raw);
  renderCart();
}
...
function removeItem(id) {
  const raw = getCartRaw().filter(i => String(i.id) !== String(id));
  saveCartRaw(raw);
  renderCart();
...
function clearCart() {
  saveCartRaw([]);
  renderCart();
  if (typeof showToast === 'function') showToast('سبد خرید خالی شد');
}
...
function renderCart() {
  ...
  if (!items.length) {
      ...
      return `
        <div class="cart-item">
          ...
          <div class="cart-item-info">
            ...
            <div class="unit-price">${toFa(p.price)} تومان</div>
          </div>
          ...
          <div style="display:flex;align-items:center;gap:8px">
            <div class="cart-item-total">${toFa(line)} ت</div>
            <button class="cart-item-remove" data-remove="${it.id}" title="حذف"><i class="fas fa-trash-alt"></i></button>
          </div>
        </div>`;
    }).join('');
    ...
    list.querySelectorAll('[data-plus]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        const cur = getAggregated().find(x => String(x.id) === String(id));
        setQty(id, (cur ? cur.qty : 0) + 1);
      });
    });
  ...
  if (discRow) {
    if (discount > 0) {
      ...
      if (discText) discText.textContent = '−' + toFa(discount) + ' تومان';
    } else {
...
function renderCheckoutSummary() {
  ...
  if (box) {
    box.innerHTML = items.map(it => {
      const p = CATALOG[it.id] || { name: 'محصول', price: 0 };
      return `<div class="summary-row"><span>${p.name} × ${it.qty}</span><span>${toFa(p.price * it.qty)} ت</span></div>`;
    }).join('');
  }
  ...
  if (coDiscRow) {
    if (discount > 0) {
      coDiscRow.style.display = 'flex';
      if (coDisc) coDisc.textContent = '−' + toFa(discount) + ' تومان';
...
function showCheckout() {
  ...
  try {
    const u = JSON.parse(localStorage.getItem('plato_user') || 'null');
    if (u && u.name) document.getElementById('fullName').value = u.name;
  } catch(e) {}
...
function processPayment() {
  ...
  if (!name || !phone || !city || !address) {
    if (typeof showToast === 'function') showToast('لطفاً اطلاعات گیرنده را کامل وارد کنید');
    return;
  }
  ...
  icon.className = 'pay-icon loading';
  ...
  // Simulate redirect to gateway then success (demo – no real payment API)
  setTimeout(() => {
    title.textContent = 'تأیید پرداخت در ' + gwName;
    msg.textContent = 'مبلغ ' + toFa(total) + ' تومان در حال پردازش است...';
  }, 1200);

  setTimeout(() => {
    icon.className = 'pay-icon success';
    icon.innerHTML = '<i class="fas fa-check"></i>';
    title.textContent = 'پرداخت موفق!';
    msg.textContent = 'سفارش شما با موفقیت ثبت شد و به‌زودی ارسال می‌شود.';
    code.style.display = 'block';
    code.textContent = 'کد پیگیری: ' + orderId;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('clearCartBtn')?.addEventListener('click', () => {
    if (getAggregated().length && confirm('سبد خرید خالی شود؟')) clearCart();
  });
  ...
  document.getElementById('applyCoupon')?.addEventListener('click', () => {
    ...
    if (code === 'PLATO10') {
      ...
      discountCode = code;
      if (typeof showToast === 'function') showToast('کد تخفیف ۱۰٪ اعمال شد ✓');
      renderCart();
    } else if (code === 'COFFEE20') {
      discount = Math.round(sub * 0.2);
      discountCode = code;
      if (typeof showToast === 'function') showToast('کد تخفیف ۲۰٪ اعمال شد ✓');
      renderCart();
    } else {
      discount = 0;
      discountCode = '';
      if (typeof showToast === 'function') showToast('کد تخفیف نامعتبر است');
      renderCart();
    }
  });
